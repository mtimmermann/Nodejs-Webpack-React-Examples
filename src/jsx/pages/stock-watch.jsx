...
import appChartService from '../../js/stock-watch/chart-stock-service';

window.app = window.app || {};
window.app.chart = $.extend(appChartTheme, window.app.chart);

window.app.chart = $.extend({}, window.app.chart,
  (function () {
    ...
    const svcGetStocks = appChartService.getStocks;
    // const svcGetStocks = appChartService.getStocksCanned; // Canned service responses
    ...
    function getData(stockCode, callback) {
      ...
      svcGetStocks(stockCode, (err, dataset) => {
        if (err) callback(err);
        else {
    ...
    return {
       ...
       */
      addStockItem: (stockCode, callback) => {
        stockCode = stockCode.toUpperCase(); // eslint-disable-line no-param-reassign

        // eslint-disable-next-line prefer-arrow-callback
        const item = $.grep(stockList, function(obj) { return stockCode === obj.stockCode; });
        if (item.length === 0) {
       ...
       */
      delStockItem: (stockCode) => {
        // eslint-disable-next-line prefer-arrow-callback
        const items = $.grep(stockList, function(obj) { return obj.stockCode !== stockCode; });
       ...
       */
      getStockList: () => { // eslint-disable-line arrow-body-style
        return stockList;
      },
       ...
       */
      // init: () => { // Arrow function, this is lost
      // eslint-disable-next-line object-shorthand
      init: function() {
        if (stockList.length === 0) this.initStockList();
...
class StockWatch extends React.Component {
  ...
  componentDidMount() {
    window.app.chart.init();
    window.app.chart.setHighChartTheme();
  }
...
$(() => {
  window.app.chart.initStockList();
});
